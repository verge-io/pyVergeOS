...
class NASVolume(ResourceObject):
    ...
    def clone(self, name: str | None = None) -> dict[str, Any] | None:
        ...
        Args:
            name: Name for the cloned volume. If not provided, a default
                name is generated by the system.

        Returns:
            Clone task information dict, or None.

        Example:
            >>> result = volume.clone(name="FileShare-copy")
        """
        from typing import cast

        manager = cast("NASVolumeManager", self._manager)
        return manager.clone(self.key, name=name)
...
class NASVolumeManager(ResourceManager["NASVolume"]):
    ...
    def clone(self, key: str, name: str | None = None) -> dict[str, Any] | None:
        """Clone a NAS volume.

        Creates a copy of the volume with all its data.

        Args:
            key: Volume $key (40-character hex string) of the source volume.
            name: Name for the cloned volume. If not provided, a default
                name is generated by the system.
        ...
        Example:
            ...
            >>> result = client.nas_volumes.clone(vol.key)

            >>> # Clone with a specific name
            >>> result = client.nas_volumes.clone(vol.key, name="FileShare-copy")
        """
        params: dict[str, Any] = {}
        if name:
            params["name"] = name

        body: dict[str, Any] = {"volume": key, "action": "clone"}
        if params:
            body["params"] = params

        result = self._client._request("POST", "volume_actions", json_data=body)
