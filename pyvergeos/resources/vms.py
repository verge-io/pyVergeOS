...
# VergeOS hotplugdrive accepts only these interfaces, and only for disks.
# ide, ahci, nvme, and non-disk media are refused after the drive exists,
# which leaves an offline drive on the VM (issue #169). Measured on 26.1.8.
...
class VM(ResourceObject):
    ...
    def hotplug_drive(
        ...
        Only ``media="disk"`` on ``virtio`` or ``virtio-scsi`` can be
        hot-plugged. Other interfaces (ide, ahci, nvme) and non-disk media
        ...
        Args:
            ...
            size: Disk size in bytes. Must be a positive whole number of GiB.
                ...
                disks in GiB.
            interface: Drive interface (default "virtio-scsi"). Only
                ``virtio`` and ``virtio-scsi`` are accepted.
            media: Media type. Only ``disk`` is accepted (default "disk").
        ...
        Raises:
            ValueError: If ``media`` is not ``disk``, ``interface`` is not
                ...
                multiple of 1 GiB.
            APIError: If VergeOS rejects the hotplug action. The drive
                created for this call is deleted before the error is
                re-raised.
        ...
        """
        if media != "disk":
            raise ValueError(f"media must be 'disk' to hot-plug a drive (got {media!r})")
        if interface not in _HOTPLUG_DRIVE_INTERFACES:
            raise ValueError(
                "interface must be 'virtio' or 'virtio-scsi' to hot-plug a drive "
                f"(got {interface!r})"
            )
        ...
        return self._post_hotplug("hotplugdrive", drive.key, self.drives.delete)
    ...
    def hotplug_nic(
        ...
        Raises:
            APIError: If VergeOS rejects the hotplug action. The NIC created
                for this call is deleted before the error is re-raised.

        Note:
            The VM must be running and have allow_hotplug enabled.
        """
        nic = self.nics.create(name=name, network=network, interface=interface)
        return self._post_hotplug("hotplugnic", nic.key, self.nics.delete)
    ...
    def _post_hotplug(
        ...
        """Post a hotplug action and delete the new device if it is rejected.

        A rejected action leaves the device offline on the VM, where it comes
        online at the next boot. Delete it so a failed call does not change
        the VM, then re-raise the rejection.
        """
        try:
            result = self._manager._client._request(
                "POST",
            ...
            try:
                ...
                logger.exception(
                    "Failed to delete device %s after %s was rejected",
                    device_key,
                    action,
                )
            raise
