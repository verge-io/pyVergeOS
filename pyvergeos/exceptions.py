...
class FieldNotProjectedError(VergeError):
    ...
    absent from the row, which happens whenever the caller narrowed
    ``fields`` (issue #117).

    Before this existed, ``is_running`` answered ``False`` for a running VM
    and ``status`` answered ``"unknown"`` -- indistinguishable from a real
    answer, and exactly the guard a caller puts in front of a destructive
    operation.

    Deliberately *not* an ``AttributeError``. ``ResourceObject.__getattr__``
    is the dict fallback, and Python calls it whenever normal attribute
    lookup raises ``AttributeError`` -- so an ``AttributeError`` subclass
    raised inside a property is swallowed and re-raised as a bare "has no
    attribute", losing the diagnostic. It would also make ``hasattr()``
    answer ``False`` for a field that exists but was not fetched, which is
    the same silent lie in a new place.
