"""Projection semantics: ``all`` is not a superset, and accessors must not guess
(issue #117).

Two defects, one mechanism.

The API resolves ``fields=all`` to the resource's *own columns*. A manager's
default projection also asks for aliased traversals — ``machine#status#running
as running`` — which are not columns and are therefore never part of ``all``.
On ``nodes``, ``all`` does not even carry ``$key``. Measured on VergeOS 26.1.8:
...
The SDK made that silent twice over: a caller-supplied ``fields`` replaces the
manager's default projection wholesale, and the accessors read the result with
``self.get(name, False)`` — so a running VM reported ``is_running is False``
and ``status == "unknown"``, indistinguishable from a real answer, and exactly
the guard a caller puts in front of a destructive operation.

The absence test is exact rather than heuristic: the API always returns a key
for every field it was asked for, using a null value when there is nothing to
report. Absence therefore means "not projected" and nothing else. Verified
against every aliased field of the networks, vms, nodes, clusters, tenants and
users projections on a live system: zero absences, several nulls.
"""
...
def computed_aliases(source: str) -> set[str]:
    """Names the computed entries of a module's default projections land under."""
    return {m.group(2) for m in COMPUTED_ALIAS.finditer(source)}
...
class TestExpandProjection:
    ...
    def test_expansion_is_a_superset_of_the_default_projection(self) -> None:
        ...
        # every alias the default projection produces is still reachable
        for field in VM_DEFAULT_FIELDS:
            alias = projection_alias(field)
            if "#" in field or field == "$key":
                assert alias in expanded, alias
    ...
    def test_narrowed_projection_passes_through_untouched(self) -> None:
        # narrowing is a legitimate request; the accessors report the absence
...
class TestManagerSendsExpandedProjection:
    ...
    def test_every_manager_with_a_default_projection_declares_it(self) -> None:
        """A manager whose defaults are unreachable cannot expand ``all``."""
        undeclared: list[str] = []
...
class TestRequireProjected:
    ...
    def test_null_is_a_value_not_an_absence(self) -> None:
        # the API returns the key with a null value when there is nothing to
        # report; that is an answer, and must not raise
    ...
    def test_error_names_the_field_and_the_all_trap(self) -> None:
    ...
    def test_error_is_not_swallowed_by_the_dict_attribute_fallback(self) -> None:
        ...
        property is therefore caught by that fallback and re-raised as a bare
        "'VM' has no attribute 'is_running'", destroying the diagnostic — and
        ``hasattr()`` would quietly answer ``False`` for a field that exists
        but was not fetched.
        """
...
class TestNoAccessorInventsAJoinedValue:
    """AST tripwire: an accessor backed by a join must not supply a default.
    ...
    through a traversal is absent whenever the caller narrows ``fields``, so
    reading one with a fallback re-introduces issue #117.
    """

    def test_no_join_backed_accessor_uses_a_silent_default(self) -> None:
        offenders: list[str] = []
        for path in sorted(RESOURCES_DIR.glob("*.py")):
            source = path.read_text()
            aliases = computed_aliases(source)
            if not aliases:
                continue
            tree = ast.parse(source)
            for cls in (n for n in ast.walk(tree) if isinstance(n, ast.ClassDef)):
...
class TestAggregateProjections:
    """Aggregates are computed too, and were the second half of issue #117.

    Found by sweeping every manager on a live system rather than by reading
    the report: ``groups``, ``tasks``, ``task_scripts`` and ``nas_services``
    still lost their counts under ``all`` after the traversal fix, because
    ``count(members) as member_count`` has no ``#`` in it.
    """

    def test_aggregate_is_classified_as_computed(self) -> None:
        assert is_computed_projection("count(members) as member_count")

    def test_traversal_is_classified_as_computed(self) -> None:
