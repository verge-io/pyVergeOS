...
class TestGroup:
    ...
    def test_member_count_refuses_to_guess_when_not_projected(self) -> None:
        """'member_count' is count(members): an aggregate, not a column, so
        it is absent from any narrowed projection. 0 is a real member count
        and must not double as "not fetched" (issue #117)."""
        group = Group({"$key": 1}, MagicMock())
